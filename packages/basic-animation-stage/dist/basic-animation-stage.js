  ...
  var SelectionAnimation = function (_base) {
    ...
    _createClass(SelectionAnimation, [{
      ...
      value: function _effectTimingsForSelectionAnimation(fromIndex, toIndex) {
        ...
        var fill = 'both';
        var duration = this.selectionAnimationDuration / Math.abs(totalSteps);

        // Adjust starting point of animation based on whether we were in the
        // middle of a user-controlled drag.
        var animationFraction = fromFraction === 0 ? 0 : forward ? animationFractionFromSelectionFraction(fromFraction, duration) : animationFractionFromSelectionFraction(1 - fromFraction, duration);
        var startDelay = -animationFraction;
        ...
        var timings = items.map(function (item, itemIndex) {
          ...
          if (positionInSequence >= 0 && positionInSequence <= Math.abs(totalSteps)) {
            ...
            var delay = startDelay + (positionInSequence - 1) * duration / 2;
            var endDelay = itemIndex === wholeTo ? -duration / 2 : // Stop halfway through.
            0; // Play animation until end.
            return { duration: duration, direction: direction, fill: fill, delay: delay, endDelay: endDelay };
          } else {
            return null;
          }
        });
